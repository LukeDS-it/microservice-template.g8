...
import $package$.http.model.NamedEntity
import $package$.testcontainers.TargetContainer
...
class $name;format="Camel"$AppSpec
  ...
  Feature("The application sends a greeting") {
    Scenario("The application greets the world when called in GET") {
      val r = HttpRequest(uri = s"http://localhost:$defaultServerPort$")
      val result = http
        .singleRequest(r)
    ...
    Scenario("The application greets the person when called in POST with a name specified") {
      val result = Marshal(NamedEntity("Joe"))
        .to[RequestEntity]
        .map { e =>
          HttpRequest(uri = s"http://localhost:$defaultServerPort$", method = HttpMethods.POST, entity = e)
        }
